#pragma once

#include "framework/Actor.h"
#include "presentation/ability/overdriveCore/OverdriveCorePresentationProfile.h"

#include <SFML/Graphics/CircleShape.hpp>
#include <SFML/Graphics/ConvexShape.hpp>

namespace ly
{
	class OverdriveCoreVisualActor final : public Actor
	{
	public:
		OverdriveCoreVisualActor(
			World* world,
			const OverdriveCorePresentationProfile& profile
		);

		void Tick(float deltaTime) override;
		void Render(sf::RenderWindow& window) override;

		void SetFlightState(
			const sf::Vector2f& worldLocation,
			const sf::Vector2f& forwardDirection,
			float explosionRadius,
			float normalizedTravel
		);
		void BeginImpact(const sf::Vector2f& worldLocation, float explosionRadius);

		float GetImpactVisualDuration() const
		{
			return mDefinition.impactVisualDuration;
		}

	private:
		enum class Phase
		{
			Flight,
			Impact
		};

		void DrawFlight(sf::RenderWindow& window);
		void DrawImpact(sf::RenderWindow& window);
		sf::Vector2f NormalizeOrDefault(const sf::Vector2f& direction) const;

		OverdriveCoreVisualDefinition mDefinition;
		ExplosionType mExplosionType = ExplosionType::Small;
		sf::ConvexShape mTrail;
		sf::CircleShape mBody;
		sf::CircleShape mGlow;
		sf::CircleShape mImpactRing;
		sf::Vector2f mForwardDirection{ 0.f, -1.f };
		float mExplosionRadius = 1.f;
		float mNormalizedTravel = 0.f;
		float mPhaseAge = 0.f;
		Phase mPhase = Phase::Flight;
	};
}
