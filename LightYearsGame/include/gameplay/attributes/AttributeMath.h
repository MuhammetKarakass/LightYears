...
namespace ly::AttributeMath
{
	...
	inline float ApplyLuckToProcChance(float baseProcChance, float luckRating, float luckScaling = 1.f)
	{
		const float baseChance = std::clamp(baseProcChance, 0.f, 1.f);
		const float contribution = std::clamp(GetCombatLuckFactor(luckRating) * std::max(0.f, luckScaling), 0.f, 1.f);
