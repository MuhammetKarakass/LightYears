...
### 2.1 Current implementation checkpoint — 15 September 2026

The arena foundation is now usable as a vertical slice: camera/world HUD split,
ThrustDrift movement, arena bounds warning, velocity look-ahead, Dash camera
follow/zoom, player restart lifecycle, three data-driven generic enemy types,
deterministic wave progression and enemy stat variation are implemented runtime
paths. The active ability runtime and typed presentation contract are also
implemented infrastructure.

The immediate gap is encounter presentation: the runtime now exposes a
side-effect-free encounter snapshot, but the minimal wave/countdown HUD that
consumes it is the next piece. Economy, purchase/restore correctness, content
acquisition/evolve selection and chapter-level run progression remain later
work. JSON registration counts are inventory only; they do not prove every
ability is playable end to end.
...
- ✅ Arena restart destroys the old ship, clears respawn callback/timer state,
  resets PlayerManager and starts a fresh player encounter.
- ✅ Player death fails the active arena encounter while preserving external
  actors; restart recreates an owned encounter.
- ✅ Encounter HUD parity: `EncounterHUDController` snapshot contract'ını
  tüketen minimal visual consumer olarak Arena'ya bağlıdır.

### Milestone 2: Arena Encounter Prototype — current vertical slice
...
- Three generic profile-backed enemies use Approach, Strafe and HoldRange
  behavior with authored Energy, Kinetic and Cryo weapons.
- Encounter runtime provides wave order, spawn interval, own-enemy clear gate,
  inter-wave delay, restart/reset, capped count/level progression and
  deterministic per-spawn variation context.
- Enemy level growth applies Health, Armor, Shield and outgoing damage through
  existing owner attribute, ShipRuntime and CombatRuntime boundaries.

Next work:

- Playtest the three-wave arena and record time-to-clear, damage intake,
  reload pressure and death rate before changing balance.
...
Before expanding encounter content, complete the snapshot-driven HUD and run
the first measured arena playtests. Scrap/ability purchase economy is not part
of the immediate encounter presentation piece.
...
- Historical verification note: the completed Debug build linked the SAS
  library, game and test executable; CTest passed
  `LightYearsGasLiteCore` and `LightYearsEngineLifetime` (2/2). This is not
  current build evidence; the 20 September 2026 docs-only audit did not run
  build/test.
