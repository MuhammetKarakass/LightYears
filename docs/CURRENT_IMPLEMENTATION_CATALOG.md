...
| Proje geneli gameplay tag sözleşmesi | ✅ | Domain kökleri/biçim doğrulaması ile iki ortak action lock (`State.ActionLock.AbilityActivation`, `State.ActionLock.PrimaryWeaponFire`); feature leaf'leri yerel kalır | LightYearsGame/include/gameplay/tags/GameplayTagSchema.h; ability/effect/actor/weapon/attachment ve ship progression validation | `LightYearsGameplayTagSchemaTests` domain ve lock kayıtlarını doğrular; action lock tüketimi GameAbility activation gate'indedir |
